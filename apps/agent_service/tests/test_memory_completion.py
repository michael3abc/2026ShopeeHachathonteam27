...
def test_memory_join_empty_roundtrip_and_nonempty_guard(replay_engine):
    ...
    if replay_engine.dialect.name == "postgresql":
        cfg.output_buffer = StringIO()
        command.downgrade(cfg, "0002_memory_completion:0001_memory_replay", sql=True)
    ...
    with replay_engine.connect() as connection:
        assert connection.scalar(text("SELECT version_num FROM agent_service_alembic_version")) == "0002_memory_completion"
