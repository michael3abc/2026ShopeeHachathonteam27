        ...
        with engine.begin() as connection:
            for index in range(2):
                connection.execute(
                    text(
                        "INSERT INTO cases "
                        "(case_ref, thread_id, order_ref, user_ref, status, created_at, updated_at) "
                        "VALUES (:case_ref, :thread_id, 'ORDER-1', 'demo_customer', "
                        "'OBSERVING', :created_at, :updated_at)"
                    ),
                    {
