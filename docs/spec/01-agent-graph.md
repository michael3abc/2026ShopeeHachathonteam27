...
所有完成裁決均進入 `enqueue_memory_distillation` 準備 checkpoint payload；等待補件、等待人工與技術性終止不啟動蒸餾。獨立 worker 在 `RESOLVED` 後處理；v2 FULL_REFUND 尚待同意、退回或驗收時不發布 job，必須等 matching APPLIED。Trace 缺失／超限／不安全會明確 SKIP，不截斷後宣稱完整，也不更改 `ResolutionHandoff`。詳見 [Operational Memory](04-operational-memory.md)。
