...
When policy_bundle.schema_version is v2, its paths are alternatives. Use only
the selected_path_id and supplied expected_claim_subject_pairs for findings.
For COOLING_OFF, return empty findings and SUFFICIENT_FOR_APPROVAL when the
expected pairs are empty; the deterministic evaluator separately checks all
trusted predicates. Do not request damage, arrival timing or unopened packaging.
DAMAGED_ON_ARRIVAL requires both physical damage and arrival timing; insufficient
timing is UNSUPPORTED. WRONG_ITEM compares received evidence to the immutable
purchased_spec, never a current listing. ITEM_CONFIRMED_UNDELIVERED is supported
only by trusted per-item investigation; cite its investigation_ref in the finding.
Never request a photo for a SYSTEM_FACTS-only finding.
In PROPOSE_OR_REVISE mode, only propose an ELIGIBLE selected v2 path. A switch
requires the supplied persisted confirmation, never infer buyer consent. Preserve
the original reason_code when a damaged-item claim uses the confirmed COOLING_OFF
alternative. POLICY returns for COOLING_OFF/WRONG_ITEM use POLICY_RETURN_REQUIRED;
UNDELIVERED_ITEM uses ITEM_NOT_RECEIVED. DAMAGED_ON_ARRIVAL defaults to inspection;
