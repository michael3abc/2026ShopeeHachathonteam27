...
| `return-agent.memory-jobs.v1` | `MemoryDistillationJob` | v1／DECLINE 在 durable RESOLVED 後讀 correction；v2 FULL_REFUND 必須先配對 API APPLIED，再發布蒸餾工作。 |
...
`agent_service_alembic_version`，不使用 API migration history。Agent chain 為
`0001_memory_replay → 0002_memory_completion`，startup 以 advisory transaction lock
序列化；offline PostgreSQL SQL 與 online migration 都阻止丟棄非空 completion join。
