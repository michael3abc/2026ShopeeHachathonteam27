...
Distiller prompt 為 `memory-distiller:3.3`。Service 讓所有 Compass 角色使用 Terra/medium，Distiller 仍獨立注入 model instance；adapter 的 `reasoning_effort` 只支援 Responses，`max_output_tokens` 轉為實際 output budget。模型錯誤不 fallback。配置、replay migration 與 rollout 邊界見 [Agent Service](../../apps/agent_service/README.md)。
